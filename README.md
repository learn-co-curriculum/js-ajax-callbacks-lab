...
1. Create a form that contains a text input field with an ID of `searchTerms` and a submit input button with an ID of `searchRepositories`. In your `$(document).ready()`, assign an event listener that invokes the `searchRepositories()` function when the `input#searchRepositories` button is clicked. When `searchRepositories()` is invoked, the value entered into the `input#searchTerms` field should be used to query the GitHub repository search API.
